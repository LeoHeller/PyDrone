...
class Client(HandleSockets):
    '''Helper class to start the client

    Arguments:
        HandleSockets {class} -- Main Sockets class
    '''

    def __init__(self, ip, port, password, mode, on_message):
        super().__init__(ip, port, password, mode,on_m = on_message)
    

    def send(self, msg):
        super().send(msg)

class Server(HandleSockets):
    '''Helper class to start the server
